...
select enable_rls_on_tx_table('group');
...
alter table only user_group
    ...
    add constraint user_group_group_id foreign key (group_id) references "group" (id);
...
alter table only group_privilege
    ...
    add constraint group_privilege_group_id foreign key (group_id) references "group" (id),
...
insert into "group"(uuid, name, version, organisation_id, audit_id)
...
select uuid_generate_v4(),
       u.id,
       (select g.id from "group" g where u.organisation_id = g.organisation_id and g.name = 'Everyone'),
       0,
       u.organisation_id,
       create_audit()
from users u;
...
select uuid_generate_v4(),
       ...
       pr.program_id,
       case
           when pr.program_id notnull then e.encounter_type_id
           else null end,
       case
           when pr.program_id isnull then e.encounter_type_id
           else null end,
       cd.id,
       0,
       s.organisation_id,
...
from operational_subject_type s
         left join operational_encounter_type e on e.organisation_id = s.organisation_id
         left join operational_program pr on pr.organisation_id = s.organisation_id
         left join checklist_detail cd on s.organisation_id = cd.organisation_id
         left join "group" g on g.organisation_id = s.organisation_id
