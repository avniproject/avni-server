-- avniproject/avni-server#1030 | STANDALONE revert of migration V1_406 -- restores the #1029/V1_405 OR-based
-- policy shape, for A/B performance testing on the clone (and as the emergency prod rollback for V1_406).
--
-- WHAT IT DOES
--   Runs the exact V1_405 body (below, verbatim) in one transaction. Its catalog-driven selection targets every
--   *_orgs policy whose USING references rls_visible_org_ids -- which matches the V1_406 COALESCE shape -- and
--   re-applies the pre-17.0 OR-based USING. Round-trip (apply -> revert -> re-apply) verified: 91/91 policies
--   each way, zero tx/ref visibility-class drift.
--
--   Deliberately left in place (harmless, matches the deployed-revert state): the rls_visible_org_ids*
--   functions (unused by the OR shape) and the V1_404 + group_privilege indexes (the OR shape cannot use them,
--   which is exactly the plan difference being measured).
--
-- RUN AS the table/policy owner (openchs) or a superuser:  psql -h <host> -d <db> -U openchs -f <this file>
--
-- FLYWAY NOTE: if this is used as an emergency rollback on a DB where Flyway has recorded V1_406, a later
-- redeploy will NOT re-run V1_406 (already recorded) -- re-apply manually with rls_1030_apply_v1_406.sql or a
-- follow-up migration when ready.

\set ON_ERROR_STOP on
BEGIN;

-- ======================= verbatim body of V1_405__RevertIndexableRLSOrgPolicies.sql ==========================
...
-- ====================================== end of verbatim V1_405 body ==========================================

COMMIT;

-- Post-revert verification: expect or_shape = total (91 on current prod), fn_referencing = 0.
SELECT count(*) FILTER (WHERE pg_get_expr(p.polqual, p.polrelid) LIKE '% OR %')                AS or_shape,
       count(*) FILTER (WHERE pg_get_expr(p.polqual, p.polrelid) LIKE '%rls_visible_org_ids%') AS fn_referencing,
       count(*)                                                                                 AS total
FROM pg_policy p
         JOIN pg_class c ON c.oid = p.polrelid
         JOIN pg_namespace n ON n.oid = c.relnamespace
