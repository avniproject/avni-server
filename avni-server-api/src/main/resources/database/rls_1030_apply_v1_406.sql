-- avniproject/avni-server#1030 | STANDALONE apply of migration V1_406 for prod-scale performance testing
-- (a writable clone restored from a prod snapshot), WITHOUT deploying the application.
--
-- WHAT IT DOES
--   Runs the exact V1_406 body (below, verbatim) in one transaction: switches every *_orgs policy from the
--   reverted (V1_405) OR-based USING to the single-eval InitPlan shape
--   `organisation_id = ANY (COALESCE((SELECT rls_visible_org_ids...()), '{}'::integer[]))`,
--   and creates the group_privilege organisation_id index.
--
-- PRECONDITIONS
--   * The DB is on the #1029/V1_405 reverted shape (any clone of current prod is). If any policy is still on
--     the #1008 `= ANY(fn())` shape, the script ABORTS atomically -- nothing to clean up.
--   * Run as the table/policy owner (openchs) or a superuser:  psql -h <host> -d <db> -U openchs -f <this file>
--   * NOT on a streaming replica (hot standby rejects DDL); use the restored clone.
--
-- FLYWAY SAFETY
--   schema_version is NOT touched. If the application (17.0.1) is later deployed against this DB, Flyway runs
--   the real V1_406 and it no-ops cleanly (policy selection matches nothing, CREATE INDEX IF NOT EXISTS skips,
--   trailing assertion passes) and records itself. Manual apply and real deploy compose safely.
--
-- REVERT: rls_1030_revert_v1_406.sql (restores the V1_405 OR-based shape; round-trip verified).
-- MEASURE: rls_1030_plan_validation.sql (run BEFORE this script for baseline, and again AFTER).

\set ON_ERROR_STOP on
BEGIN;

-- ======================= verbatim body of V1_406__IndexableRLSOrgPoliciesSingleEval.sql =======================
...
-- ====================================== end of verbatim V1_406 body ==========================================

COMMIT;

-- Post-apply verification: expect coalesce_shape = total (91 on current prod), or_shape = 0.
SELECT count(*) FILTER (WHERE pg_get_expr(p.polqual, p.polrelid) LIKE '%COALESCE%'
                          AND pg_get_expr(p.polqual, p.polrelid) LIKE '%rls_visible_org_ids%') AS coalesce_shape,
       count(*) FILTER (WHERE pg_get_expr(p.polqual, p.polrelid) LIKE '% OR %')                AS or_shape,
       count(*)                                                                                 AS total
FROM pg_policy p
         JOIN pg_class c ON c.oid = p.polrelid
         JOIN pg_namespace n ON n.oid = c.relnamespace
