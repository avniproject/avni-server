...
create or replace function check_form_mapping_uniqueness(organisationId int, subjectTypeId int, entityId bigint, observationsTypeEntityId int, taskTypeId int,
                                                         formId bigint,
...
begin
    if exists (select * from public.form f where (
            (
                ...
                or
                (f.form_type = 'ProgramEncounter' and (entityId is null or observationsTypeEntityId is null))
                or
                (f.form_type = 'ProgramEncounterCancellation' and (entityId is null or observationsTypeEntityId is null))
                or
                (f.form_type = 'Encounter' and (entityId is not null or observationsTypeEntityId is null))
                or
                (f.form_type = 'IndividualEncounterCancellation' and (entityId is not null or observationsTypeEntityId is null))
            )
