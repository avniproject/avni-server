...
public class BaseSubjectSearchQueryBuilder<T> {
    ...
    protected static final String PROGRAM_ENCOUNTER_FILTER = "i.id in (select penc.individual_id from program_encounter penc where penc.organisation_id = " + ORGANISATION_ID_PLACEHOLDER + " and penc.encounter_date_time is not null and penc.is_voided is false and exists (select 1 from program_enrolment penr where penr.id = penc.program_enrolment_id and penr.is_voided is false)\n";
